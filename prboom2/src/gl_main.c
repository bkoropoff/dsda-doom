...
void gld_AddWall(gl_wall_t *wall)
{
  ...
  else /* twosided */
  {
    ...
    if (temptex && sidedef->midtexture != NO_TEXTURE && backsector->ceilingheight>frontsector->floorheight)
    {
      ...
      scaled_texheight = (fixed_t) ((item.gltexture->realtexheight << FRACBITS) / item.yscale);
      scaled_rowoffset = (fixed_t) ((sidedef->rowoffset + sidedef->rowoffset_mid) / item.yscale);
      ...
      // FIXME: only render item as translucent if it contains holes
      trans = item.alpha != 1.0f || dsda_IntConfig(dsda_config_gl_filter);
      gld_AddDrawWallItem(trans ? GLDIT_TWALL : GLDIT_MWALL, &item);
      item.alpha = 1.0f;
    }
bottomtexture:
    /* bottomtexture */
