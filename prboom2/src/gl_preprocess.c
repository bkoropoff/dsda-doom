...
static void gld_SetupSubsectorLoop(subsector_t *ssec, int numedgepoints)
{
  GLLoopDef **loop;
  int *loopcount;

  loop = &subsectorloops[ssec - gl_rstate.subsectors].loops;
  loopcount = &subsectorloops[ssec - gl_rstate.subsectors].loopcount;
...
static void gld_TesselateChunk(chunkmeta_t* chunkmeta)
{
  GLUtesselator* tess;
  tessctx_t ctx;
  int i;
...
static void gld_GetSubSectorVertices(void)
{
  ...
  {
    subsector_t* ssector = &gl_rstate.subsectors[i];

    numedgepoints = ssector->numlines;

    gld_AddGlobalVertexes(numedgepoints);
    gld_SetupSubsectorLoop(ssector, numedgepoints);

    for (j = 0; j < numedgepoints; j++)
    {
      seg_t* seg = &gl_rstate.segs[ssector->firstline + j];
      vbo_xyz_uv_t* vbo = &flats_vbo[gld_num_vertexes++];
...
static void gld_PreprocessSectors(void)
{
  ...
  if (gl_rstate.numsubsectors)
  {
    subsectorloops=Z_Malloc(sizeof(*subsectorloops)*gl_rstate.numsubsectors);
    if (!subsectorloops)
      I_Error("gld_PreprocessSectors: Not enough memory for array subsectorloops");
    memset(subsectorloops, 0, sizeof(*subsectorloops)*gl_rstate.numsubsectors);
  }
  ...
  for (i = 0; i < gl_rstate.numchunks; ++i)
  {
    ...
    if (chunkmeta->numpoints)
      // If path generation succeeded, GLU tessellate the entire perimeter
      gld_TesselateChunk(chunkmeta);
    else
    {
      // Fall back on concatenating all the subsector loops
      GLLoops* cloops = &chunkloops[chunk - gl_rstate.chunks];
      subsector_t* sub;
      submeta_t* meta;
      int subnum;
      int loops = 0;
      ...
      for (subnum = chunkmeta->subsectors; subnum != -1; subnum = meta->chunk_next)
      {
        ...
        loops += subsectorloops[sub - gl_rstate.subsectors].loopcount;
      }

      if (loops)
      {
        cloops->loops = Z_Calloc(loops, sizeof(*cloops->loops));
        if (!cloops->loops)
          I_Error("Could not allocate memory for chunk loops");
        cloops->loopcount = loops;

        loops = 0;
        for (subnum = chunkmeta->subsectors; subnum != -1; subnum = meta->chunk_next)
        {
          GLLoops* glsub = &subsectorloops[subnum];

          memcpy(&cloops->loops[loops], glsub->loops, glsub->loopcount * sizeof(*glsub->loops));
          loops += glsub->loopcount;
  ...
  gld_MarkLoopsForClamp(subsectorloops, gl_rstate.numsubsectors);
...
void gld_PreprocessLevel(void)
{
  ...
  if (!gl_preprocessed)
  {
    ...
    numsubsectors_prev = gl_rstate.numsubsectors;
