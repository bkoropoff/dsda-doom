...
static void AnnotateSubsectorRender(subsector_t* sub)
{
  ...
  // Add two-sided segs first, so if there are overlapping lines
  // in the subsector (mapping trick), the single-sided segs don't
  // occlude them.
  ...
  // Add two-sided segs first, so if there are overlapping lines
  // in the subsector (mapping trick), the single-sided segs don't
  // occlude them.  Example map: Stickney E1M2
  for (i = 0; i < sub->numlines; ++i)
  {
    seg_t* seg = &gl_rstate.segs[sub->firstline + i];
