                          ...
                          const chunkmeta_t* chunkmeta)
{
  ...
  DumpKey(fp, "portals");
  fprintf(fp, "[");
  for (i = 0; i < comp->numportals; ++i)
  {
    DumpId(fp, comp->firstportal + i);
    if (i != comp->numportals - 1)
      DumpComma(fp);
  }
  fprintf(fp, "]");
  DumpComma(fp);
...
static void DumpPortal(FILE* fp, const portal_t* portal)
{
  fprintf(fp, "{");
  DumpKeyId(fp, "chunk", portal->chunk);
  DumpComma(fp);
  DumpKey(fp, "bbox");
  DumpBBox(fp, portal->bbox);
  fprintf(fp, "}");
}

static void DumpPortals(FILE* fp)
{
  int i;

  fprintf(fp, "{");
  for (i = 0; i < gl_rstate.numportals; ++i)
  {
    DumpIdKey(fp, i);
    DumpPortal(fp, &gl_rstate.portals[i]);
    if (i != gl_rstate.numportals - 1)
      DumpComma(fp);
  }
  fprintf(fp, "}");
}
