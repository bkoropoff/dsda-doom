...
typedef enum {
  PORTF_NONE = 0x0,
  // Can see through even if backsector height is less than or equal to
  // frontsector height
  PORTF_CLEAR_TOP = 0x1,
  // Can see through even if frontsector height is less than or equal to
  // backsector height
  PORTF_CLEAR_BOTTOM = 0x2
} portal_flags_t;

// GL: portal into another chunk
typedef struct
{
  int chunk;
  portal_flags_t flags;
  // Bounding box of portal for visibility tests.
  // All segs leading to the same chunk are coalesced into a single box for
  // efficiency.
  fixed_t bbox[4];
} portal_t;
...
typedef struct
{
  // Containing sector
  sector_t* sector;
  // GL: first and number of portals in gl_rstate.portals
  int numportals, firstportal;
  // GL: first and number of walls in gl_rstate.walls
