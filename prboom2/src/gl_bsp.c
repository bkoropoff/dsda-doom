...
static void AddPolyWalls(polyobj_t* poly, gl_polyobj_t *gpo)
{
  int i;
  ...
  for (i = 0; i < gpo->numwalls; ++i)
    gld_AddWall(&gl_rstate.walls[gpo->firstwall + i]);
...
static void RenderDeferred(void)
{
  ...
  for (i = 0; i < numdeferred; ++i)
  {
    ...
    // is rendered.  This avoids a problem on grove.wad
    // FIXME: find a better way to avoid this
...
static void RenderPolyObjs(void)
{
  ...
  for (i = 0; i < po_NumPolyobjs; ++i)
    AddPolyWalls(&polyobjs[i], &gl_rstate.polyobjs[i]);
}
