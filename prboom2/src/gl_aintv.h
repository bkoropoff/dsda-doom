...
// Return magnitude (subtended angle) of interval
// Has to return uint64 to distinguish 0 and 360 degrees
static inline uint64_t GL_AIntvMagnitude(const gl_aintv_t* a)
{
  if (GL_AIntvIsFull(a))
    return (uint64_t) ANGLE_MAX + 1;
